...
const ScoreRing = ({ score }) => {
  ...
  let bgGradient = "from-red-50 to-red-100/40";
  ...
  if (normalizedScore >= 85) {
    ...
    strokeColor = "#EA580C";
    bgGradient = "from-orange-50 to-orange-100/40";
  ...
  return (
    <div className={`relative flex items-center justify-center w-44 h-44 rounded-full bg-gradient-to-br ${bgGradient}`}>
      <svg className="w-40 h-40 transform -rotate-90" viewBox="0 0 100 100">
        {/* Track */}
        <circle cx="50" cy="50" r="40" stroke="#E5E7EB" strokeWidth="7" fill="none" />
        {/* Fill */}
        <motion.circle
          cx="50" cy="50" r="40"
          stroke={strokeColor}
...
const ResumeEvaluator = ({ onEvaluationComplete, onEvaluationReset }) => {
  ...
  return (
    <div className="min-h-[calc(100vh-80px)] bg-[#FAFAFA] font-['Inter',sans-serif] flex items-start justify-center p-6 pt-10">
        ...
        <AnimatePresence mode="wait">
              ...
              <div className="px-10 py-8 border-b border-gray-100 bg-gray-50/50 flex items-center justify-between">
                <div>
                  <h1 className="text-2xl font-black text-black tracking-tight flex items-center gap-3">
                    <span className="material-symbols-outlined text-[#6A0DAD] text-3xl">document_scanner</span>
                    AI Resume Evaluator
                  </h1>
              ...
              <div className="p-10">
                <div className="grid md:grid-cols-2 gap-8">
                  ...
                  <div className="flex flex-col h-full">
                    ...
                    <textarea
                      className="flex-grow w-full min-h-[160px] p-5 rounded-2xl bg-gray-50 border border-gray-200 text-sm text-gray-800 placeholder:text-gray-400 focus:bg-white focus:ring-2 focus:ring-[#6A0DAD]/30 focus:border-[#6A0DAD] outline-none transition-all resize-none"
                  ...
                  <div className="flex flex-col h-full">
                    ...
                    <div className="flex-grow relative border-2 border-dashed border-purple-200 bg-purple-50/30 hover:bg-purple-50 rounded-2xl transition-all flex flex-col items-center justify-center p-6 text-center cursor-pointer group min-h-[160px]">
                      ...
                      <label htmlFor="resume-upload" className="cursor-pointer flex flex-col items-center justify-center w-full h-full">
                        <div className={`w-12 h-12 rounded-xl flex items-center justify-center mb-3 transition-transform ${fileName ? "bg-green-100 text-green-600" : "bg-white shadow-sm text-[#6A0DAD] group-hover:scale-110"}`}>
                          <span className="material-symbols-outlined text-2xl">{fileName ? "task" : "cloud_upload"}</span>
                        </div>
                        <p className="text-black font-extrabold text-sm">{fileName ? fileName : "Click to Upload PDF"}</p>
                        <p className="text-gray-400 text-xs font-medium mt-1">PDF only · Max 2MB</p>
                      </label>
                    </div>
                  </div>
                </div>
                ...
                {error && (
                  <div className="mt-6 p-4 rounded-xl bg-red-50 border border-red-100 text-red-600 text-sm font-bold flex items-center gap-2">
                ...
                <button
                  ...
                  className="w-full mt-8 py-5 rounded-2xl font-extrabold text-white bg-[#6A0DAD] hover:bg-[#580b94] transition-all shadow-[0_8px_20px_rgba(106,13,173,0.2)] active:scale-95 flex justify-center items-center gap-3 text-lg"
                >
                  Analyze Resume
          ...
          {loading && (
            <motion.div
              ...
              initial="hidden" animate="visible" exit="exit"
              variants={pageTransition}
              className="bg-white rounded-[2.5rem] shadow-xl border border-gray-100 p-16 flex flex-col items-center justify-center text-center"
            >
              <div className="relative w-24 h-32 bg-gray-50 border-2 border-gray-200 rounded-xl overflow-hidden mb-8 shadow-sm">
                <motion.div
                  animate={{ y: ["0%", "400%"] }}
                ...
                <div className="p-3 space-y-2 opacity-30">
                  ...
                  <div className="h-2 bg-gray-400 rounded w-full mt-4" />
                </div>
              </div>
              <h2 className="text-2xl font-black text-black animate-pulse">Running AI Analysis</h2>
              <p className="text-gray-500 font-medium mt-2">Cross-referencing your experience with the job description...</p>
            </motion.div>
          )}
          ...
          {result && !loading && (
            ...
            >
              ...
              <motion.div
                variants={fadeUp}
                className="flex flex-col sm:flex-row items-center justify-between gap-4 bg-white px-8 py-5 rounded-[2rem] shadow-sm border border-gray-100"
              >
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    <p className="text-sm font-bold text-black tracking-tight">{fileName || "Your Resume"}</p>
                    <p className="text-xs text-gray-500 font-medium">Evaluation complete</p>
                  </div>
                </div>
                <button
                  onClick={handleResetSession}
                  className="px-5 py-2.5 rounded-full font-bold text-gray-700 bg-gray-100 hover:bg-gray-200 transition-all active:scale-95 flex items-center gap-2 text-sm"
                >
              ...
              <motion.div variants={fadeUp} className="grid md:grid-cols-3 gap-5">
                ...
                {/* ATS Score */}
                <div className="md:col-span-1 bg-white rounded-[2rem] shadow-sm border border-gray-100 p-8 flex flex-col items-center justify-center text-center">
                  <ScoreRing score={result.atsScore} />
                  <h3 className="text-lg font-black text-black mt-6">ATS Match Score</h3>
                ...
                {/* Actionable Fixes */}
                <div className="md:col-span-2 bg-white rounded-[2rem] shadow-sm border border-gray-100 p-8 flex flex-col">
                  <div className="flex items-center gap-3 mb-5 pb-4 border-b border-gray-100">
                    <span className="material-symbols-outlined text-[#6A0DAD] text-2xl">fact_check</span>
                    <div>
                      <h3 className="text-lg font-black text-black leading-tight">Actionable Fixes</h3>
                      <p className="text-xs text-gray-400 font-medium mt-0.5">Specific steps to improve your match rate</p>
                    </div>
                  </div>
                  ...
                  <div className="flex-grow space-y-3 max-h-[280px] overflow-y-auto pr-1 custom-scrollbar">
                    {result.suggestions && result.suggestions.length > 0 ? (
                      result.suggestions.map((tip, index) => (
                        <div
                          key={index}
                          className="p-4 rounded-2xl bg-gray-50 border border-gray-100 flex items-start gap-3.5"
                        >
                          <div className="w-6 h-6 rounded-full bg-orange-100 flex items-center justify-center text-orange-600 font-bold text-[10px] shrink-0 mt-0.5">
                            {index + 1}
                          </div>
                      ...
                      <div className="flex flex-col items-center justify-center text-center h-32 opacity-60">
                        ...
                        <p className="text-gray-800 font-bold">No major gaps found.</p>
                      </div>
                    )}
                  </div>
                </div>
              ...
              {hasKeywords && (
                ...
                >
                  <div className="flex items-center gap-3 mb-6 pb-4 border-b border-gray-100">
                    <span className="material-symbols-outlined text-rose-500 text-2xl">key</span>
                    <div>
                      <h3 className="text-lg font-black text-black leading-tight">Missing Keywords</h3>
              ...
              <motion.div variants={fadeUp}>
                <button
                  ...
                  className="w-full py-5 rounded-2xl font-extrabold text-white bg-[#6A0DAD] hover:bg-[#580b94] transition-all shadow-[0_8px_20px_rgba(106,13,173,0.18)] active:scale-95 flex justify-center items-center gap-2 text-base"
                >
